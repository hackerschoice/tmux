...
tmux send-keys -t "$p" " _dlfile=\"$r\"; case \"\$_dlfile\" in \"~\"|\"~/\"*) _dlfile=\"\$HOME\${_dlfile#\~}\";; esac; echo; printf '%s ' \"$sz_m\"; if [ -r \"\$_dlfile\" ]; then wc -c < \"\$_dlfile\"; else stty echo; set -H 2>/dev/null; printf 'ERR\ndownload: file not readable: %s\n' \"\$_dlfile\"; fi" C-m
fsize=0; i=0
while [ $i -lt 50 ]; do
...
while :; do
  ...
  [ "$idle" -ge 15 ] && { restore_win; tmux display-message "Download ERR(3): stalled (no growth 15s)"; tmux pipe-pane -t "$p"; tmux send-keys -t "$p" C-c; tmux send-keys -t "$p" " stty echo; set -H 2>/dev/null" C-m; rm -f "$t" "$x" "$o" "$v"; exit 3; }
  sleep 1
done
tmux pipe-pane -t "$p"
restore_win
